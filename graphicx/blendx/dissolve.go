...
// 随机选择一个色
func BlendDissolveColor(foreColor, backColor color.Color, factor float64, keepForegroundAlpha bool) color.Color {
...
// 这种效果对模拟破损纸的边缘或原图的 “泼溅”类型是重要的。
// 随机选择一个色
func BlendDissolveRGBA(foreR, foreG, foreB, foreA uint32, backR, backG, backB, backA uint32, factor float64, _ bool) (R, G, B, A uint32) {
	if rand.Float64() <= factor {
